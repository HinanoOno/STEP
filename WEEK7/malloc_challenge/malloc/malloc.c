...
// To find the index of the bin that the size belongs to
int calculate_bin_index(size_t size) {
  int bin_index = 0;
  size_t bin_range = 8;

  while (bin_index < BINS - 1 && size > bin_range) {
    bin_range *= MULTIPLIER;
    bin_index++;
  }
...
void my_add_to_free_list(my_metadata_t *metadata) {
  ...
  int bin_index = calculate_bin_index(metadata->size);
...
void *my_malloc(size_t size) {
  int bin_index = calculate_bin_index(size);
  ...
  while (best_fit_bin_index < BINS - 1 && best_fit->size > best_fit_bin_size) {
    best_fit_bin_size = (size_t)(best_fit_bin_size * MULTIPLIER);
    best_fit_bin_index++;
  }
