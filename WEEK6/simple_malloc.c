...
void *simple_malloc(size_t size) {
  simple_metadata_t *metadata = simple_heap.free_head;
  simple_metadata_t *prev = NULL;
  simple_metadata_t *best_fit = NULL;
  simple_metadata_t *best_fit_prev = NULL;
  ...
  printf("metadata: %p\n", best_fit);
  // Remove the free slot from the free list.
  simple_remove_from_free_list(best_fit, best_fit_prev);
...
//simple_malloc()とsimple_free()のテスト
int main() {
    // 整数配列のためのメモリを割り当てる
    int *array = (int*)simple_malloc(10 * sizeof(int));
    if (array == NULL) {
        // メモリ割り当てに失敗した場合
